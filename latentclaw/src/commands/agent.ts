...
import { resolveLatentMasConfig } from "../latent-mas/config.js";
import { runLatentMasAgent } from "../latent-mas/run.js";
...
function runAgentAttempt(params: {
  ...
  const authProfileId =
    params.providerOverride === params.primaryProvider
      ...
      : undefined;
  const latentMasConfig = resolveLatentMasConfig({
    provider: params.providerOverride,
  ...
  if (latentMasConfig) {
    const hasUnsupportedLatentInputs =
      Boolean(params.opts.images?.length) || Boolean(params.opts.clientTools?.length);
    if (hasUnsupportedLatentInputs && normalizeProviderId(params.providerOverride) === "latentmas") {
      throw new Error(
        "LatentMAS models currently support text-only turns without client tools or image inputs.",
      );
    }
    if (!hasUnsupportedLatentInputs) {
      return runLatentMasAgent({
        sessionId: params.sessionId,
        sessionFile: params.sessionFile,
        workspaceDir: params.workspaceDir,
        cfg: params.cfg,
